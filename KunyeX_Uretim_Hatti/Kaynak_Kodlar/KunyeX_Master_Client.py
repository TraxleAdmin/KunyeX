...
class KunyeXPremiumClient(ctk.CTk, TkinterDnD.DnDWrapper):
    ...
    def fade_in(self):
        if self.fade_val < 1.0:
            self.fade_val += 0.05
            self.splash.attributes("-alpha", self.fade_val)
    ...
    def show_update_dialog(self, new_version, exe_download_link):
        ...
        upd.title("Sistem Güncellemesi")
        upd.geometry("450x300")
        upd.attributes("-topmost", True)
        upd.configure(fg_color=BG_COLOR)
        upd.grab_set() 
        
        ctk.CTkLabel(upd, text="YENİ GÜNCELLEME", font=("Helvetica", 20, "bold"), text_color=TEXT_COLOR).pack(pady=(30, 10))
        ctk.CTkLabel(upd, text=f"Sürüm: {self.APP_VERSION} -> {new_version}", text_color=TEXT_MUTED, font=("Helvetica", 12)).pack()
        
        progress_bar = ctk.CTkProgressBar(upd, width=350, height=8, progress_color=ACCENT_COLOR)
        progress_bar.set(0)
        
        status_lbl = ctk.CTkLabel(upd, text="İndirmeye hazır.", text_color=TEXT_COLOR, font=("Helvetica", 11))
        
        btn = ctk.CTkButton(upd, text="GÜNCELLE", width=250, height=45, corner_radius=6, fg_color=ACCENT_COLOR, text_color=TEXT_COLOR, font=("Helvetica", 14, "bold"))
        ...
        def download_and_patch():
            ...
            def worker():
                try:
                    ...
                    with urllib.request.urlopen(req, context=ctx) as response:
                        ...
                        with open(temp_exe, 'wb') as out_file:
                            while True:
                                ...
                                upd.after(0, lambda p=downloaded/total_size: progress_bar.set(p))
                                upd.after(0, lambda p=int((downloaded/total_size)*100): status_lbl.configure(text=f"Yükleniyor... %{p}"))
                                
                    upd.after(0, lambda: status_lbl.configure(text="Yeniden başlatılıyor...", text_color=SUCCESS_COLOR))
                    time.sleep(1)
                    ...
                    upd.after(0, lambda err=str(e)[:35]: status_lbl.configure(text=f"Hata: {err}", text_color=DANGER_COLOR))
                    upd.after(0, lambda: btn.configure(state="normal", text="TEKRAR DENE"))
        ...
        upd.protocol("WM_DELETE_WINDOW", self.quit_app)
    ...
    def fade_main_window(self, current_alpha):
        if current_alpha < 1.0:
            ...
            self.after(15, lambda: self.fade_main_window(current_alpha))
        else:
    ...
    def add_focus_glow(self, widget):
        # 🔥 MÜDAHALE: Tıklanan kutunun parlayarak odaklanmasını sağlayan geri bildirim
    ...
    def build_login_elements(self):
        ...
        self.serial_entry = ctk.CTkEntry(self.main_frame, placeholder_text="KUNYEX-PRO-XXXX-XXXX", width=350, height=50, font=ctk.CTkFont(family="Helvetica", size=16, weight="bold"), justify="center", fg_color=SIDEBAR_COLOR, border_color=BORDER_COLOR, corner_radius=8, text_color=TEXT_COLOR)
        ...
        self.activate_btn = ctk.CTkButton(self.main_frame, text="GİRİŞ YAP", width=350, height=50, corner_radius=8, fg_color=ACCENT_COLOR, border_width=0, hover_color=HOVER_ACCENT, font=ctk.CTkFont(family="Helvetica", size=14, weight="bold"), text_color="#FFFFFF", command=self.start_verification)
    ...
    def transition_to_dashboard(self):
        ...
        self.dropzone = ctk.CTkFrame(self.sidebar, fg_color=BG_COLOR, border_width=1, border_color=BORDER_COLOR, corner_radius=8, height=180)
        self.dropzone.pack(fill="x", padx=15, pady=10)
        self.dropzone.pack_propagate(False)
        
        def d_enter(e): self.dropzone.configure(border_color=ACCENT_COLOR)
        def d_leave(e): self.dropzone.configure(border_color=BORDER_COLOR)
        ...
        self.master_print_btn = ctk.CTkButton(self.sidebar, text="PDF OLUŞTUR", height=45, corner_radius=6, fg_color=SUCCESS_COLOR, hover_color="#1A7C38", text_color="#FFFFFF", font=ctk.CTkFont(family="Helvetica", size=13, weight="bold"), command=self.open_print_options, state="disabled")
        self.master_print_btn.pack(side="bottom", fill="x", padx=15, pady=20)

        # 🔥 YENİ BUTON: ESKİ KÜNYELERİ GETİR
        self.load_memory_btn = ctk.CTkButton(self.sidebar, text="ESKİ KÜNYELERİ GETİR", height=40, corner_radius=6, fg_color="transparent", border_width=1, border_color=BORDER_COLOR, hover_color=SIDEBAR_COLOR, text_color=TEXT_COLOR, font=ctk.CTkFont(family="Helvetica", size=12, weight="bold"), command=self.load_memory_jobs)
